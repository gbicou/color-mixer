...
class Reflectance
{
     ...
     */
    const SPD_G = [
        0.49108579, 0.46944057, 0.4016578,  0.2449042,  0.0682688, 0.02732883, 0.013606,   0.01000187, 0.01284127, 0.02636635, 0.07058713, 0.70421692, 0.85473994, 0.95081565,
        0.9717037,  0.97651888, 0.97429245, 0.97012917, 0.9425863, 0.99989207, 0.99989891, 0.13823139, 0.06968113, 0.05628787, 0.06111561, 0.08987709, 0.13656016, 0.22169624,
        0.32176956, 0.36157329, 0.4836192,  0.46488579, 0.47440306, 0.4857699, 0.49267971, 0.49625685, 0.49807754, 0.49889859,
    ];
     ...
     * @var float[]
     */
    const XYZ_R = [  3.2409699419045226,  -1.537383177570094,   -0.4986107602930034  ];
    /**
     * @var float[]
     */
    const XYZ_G = [ -0.9692436362808796,   1.8759675015077202,   0.04155505740717559 ];
    /**
     * @var float[]
     */
    const XYZ_B = [  0.05563007969699366, -0.20397695888897652,  1.0569715142428786  ];
    ...
    private function calculate(ColorInterface $color): void
    {
        ...
        for ($i = 0; $i < self::SIZE; $i++) {
            $this->R[$i] = max(
                self::EPSILON,
                $weights[0]
                + $weights[1] * self::SPD_C[$i]
                + $weights[2] * self::SPD_M[$i]
                + $weights[3] * self::SPD_Y[$i]
                + $weights[4] * self::SPD_R[$i]
                + $weights[5] * self::SPD_G[$i]
                + $weights[6] * self::SPD_B[$i]
            );
        }
    }
